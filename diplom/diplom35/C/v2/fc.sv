...
always_ff @( posedge clk ) begin
  if(sel_data) begin
    din <= din_i; 
  end else if(tg_en & rco_sel== 0) begin
    din <= tang;
  end else if(tg_en & rco_sel== 1) begin
    din <= tang;
  end else begin
    din <= 0;
  end
...
typedef enum logic [NN_FSM_BUS_WIDTH-1:0] {
  ...
  NN_TANG_STRING_CALC = NN_FSM_BUS_WIDTH'('b110010),
  NN_UPD_RCO_TYPE     = NN_FSM_BUS_WIDTH'('b110000),
  NN_INIT_OUT_SYNC    = NN_FSM_BUS_WIDTH'('b111000),
  NN_INIT_OUT         = NN_FSM_BUS_WIDTH'('b111010),
  NN_OUT_CALC         = NN_FSM_BUS_WIDTH'('b101010),
  NN_OUT_RDY          = NN_FSM_BUS_WIDTH'('b101011)
} nn_fc_state_t;
...
//always_ff @(posedge clk) begin
//  tg_en_ff <= (fsm_state_ff == NN_TANG_STRING_CALC );
//end

assign rc_done   = (fc_addr == PICT_SIZE);
//assign mem_en    = (fsm_state_ff == NN_STRING_CALC      ) & (fc_addr == PICT_SIZE-1);
